  ...
  <div class="page-root">
     <div class="canvas-holder">
      ...
      <P_1_C_24
        ...
        :active-fe-fsm-state-id="orchestrator.activeFsmStates.FE_APPFSM"
        :active-be-fsm-state-id="orchestrator.activeFsmStates.BE_FSM"
    ...
    <P_1_C_84
      ...
      :dsvNodeData="dsvModalData"
       :flowState="modalFlowState" @update:visible="onCloseDsvModal"
      @execute-flow="() => orchestrator.addLog('DSV 流程执行... (未实现)', 'warn')"
      @internal-node-click="onNodeClick"
    />
    
    <P_1_C_128
      v-if="showDbModal && dbModalData"
      :visible="showDbModal"
      :dsvNodeData="dbModalData"
       :flowState="modalFlowState" @update:visible="onCloseDbModal"
...
// [ ★★★ 核心修改 ★★★ ]
// 将 activeNodeId 和 errorNodeId 分别传递
const modalFlowState = computed((): DsvFlowState => {
  return {
    status: isTracing.value ? 'running' : 'idle',
     ...
     // 黄色高亮
    highlightedNodes: [orchestrator.activeNodeId].filter(Boolean) as string[],
    highlightedLinks: [orchestrator.activeEdge].filter(Boolean) as string[],
    
    // 红色高亮
    failureNodes: [orchestrator.errorNodeId].filter(Boolean) as string[],
    failureLinks: [orchestrator.errorEdge].filter(Boolean) as string[],

    // [ ★ 核心修改 ★ ]
    // 将 orchestrator.activeFsmStates.DSV_FSM (DSV的状态) 传递给弹窗
    activeFsmStateId: orchestrator.activeFsmStates.DSV_FSM
    // [ ★ 修改结束 ★ ]
  }
})
// [ ★★★ 修正结束 ★★★ ]
